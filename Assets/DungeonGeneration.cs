...
void Start()
    {
        // Generate the dungeon map and walls
        Debug.Log("1");
 ...
 void CreateWalls()
    {
        ...
        for(int x = 0; x < width; x++)
        {
            for(int z = 0; z < height; z++)
            {
                ...
                CheckWall(x, z, 1, 0);  // East
                CheckWall(x, z, -1, 0); // West
                CheckWall(x, z, 0, 1);  // North
                CheckWall(x, z, 0, -1); // South
            }
        }
    }
...
// public public int[,] SpawnDungeon(int _sizeX,int _sizeZ, int _nbrSquareForGeneration)
// {
...
public int[,] CreateDungeonSquares(int _sizeX, int _sizeZ, int _squareNbr)
{
    ...
    for (int i = 0; i < _squareNbr; i++)
    {
        ...
        int _roomSizeX = random.Next(1, _sizeX / 5);
        int _roomSizeZ = random.Next(1, _sizeZ / 5);
...
public bool isEdgeCoordinateValid(int[,] dungeonMap,int room,int x, int z){
...
public bool IsValid(int x, int z, int sizeX, int sizeZ)
    {
        return (x >= 0) && (x < sizeX) && (z >= 0) && (z < sizeZ);
    }
public bool IsUnBlocked(int[,] dungeon, int x, int z)
    {
...
public bool IsDestination(int x, int z, IntVector2 dest)
    {
        return (x == dest.x && z == dest.z);
    }
public double CalculateHValue(int x, int z, IntVector2 dest)
    {
        // manhattan distance to goal
        return Math.Abs (x - dest.x) +  Math.Abs (z - dest.z);
    }
public Stack<Cell> astar_corridors(int[,] dungeon, IntVector2 start, IntVector2 dest){
    ...
    for (int i=0;i<sizeX;i++){
        for(int j=0;j<sizeZ;j++){
            cell_info[i, j] = new Cell(new IntVector2(i, j));
                ...
                for (int j = -1; j <= 1; j++)
                {
                    ...
                    if (IsValid(newX, newZ, sizeX, sizeZ))
                    {
                        ...
                        if (!closedList[newX, newZ] && IsUnBlocked(dungeon, newX, newZ))
                        {
                            ...
                            {
                                IntVector2 newCoords=new IntVector2(newX,newZ);
 ...
 public Stack<Cell> TracePath(Cell[,] cell_info, IntVector2 dest)
    {
        Console.WriteLine("\nThe Path is ");
        ...
        {
            path.Push(new Cell(new IntVector2(x, z)));
...
public void markCorridorsMap(int[,]dungeonMap,Stack<Cell>path){
...
public int[,] createCorridors(int[,]dungeonMap,int sizeX,int sizeZ, int roomNumber){
    int [,]newMap=new int [sizeX,sizeZ];
    System.Array.Copy(dungeonMap,newMap, sizeX*sizeZ);
    if(roomNumber>1){
        for (int i=1;i<=roomNumber;i++){
            ...
            for (int j=1;j<=roomNumber;j++){
                if(j!=i){
                    ...
                    Stack<Cell> path = new Stack<Cell>();
                    path=astar_corridors(dungeonMap,new IntVector2(xStart,zStart),new IntVector2(xEnd,zEnd));
...
void generateWalls(int[,] dungeon, GameObject wallPrefab, float tileSize)
{
    ...
    for (int x = 0; x < width; x++)
    {
        for (int z = 0; z < height; z++)
        {
            ...
            if (dungeon[x, z] == 0)
            {
                // Check if this empty space is adjacent to a room or corridor
...
// Stack<Cell> path = new Stack<Cell>();
// path=astar_corridors(dungeon,new IntVector2(0,1),new IntVector2(49,10));
}